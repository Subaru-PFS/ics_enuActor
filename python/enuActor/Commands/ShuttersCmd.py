...
class ShuttersCmd(object):

    def __init__(self, actor):
        self.actor = actor
        self.vocab = [
            ('shutters', 'status', self.status),
            ('shutters', '<cmd>', self.command),
            ('shutters', 'start [@(operation|simulation)]', self.set_mode),
            ('shutters', '@(open|close|reset) @(red|blue)', self.shutter),
            ('shutters', '@(open|close|reset)', self.shutter),
            ('shutters', '@(off|load|busy|idle|SafeStop|fail)', self.set_state),
            ('shutters', 'init', self.init),
            ('shutters', 'stop',
