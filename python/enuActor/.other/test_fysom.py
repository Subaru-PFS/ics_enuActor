...
class QThread(Thread, Fysom):
    def __init__(self):
        super(QThread, self).__init__()
        Fysom.__init__(self, MAP)
        #self.fsm = Fysom(MAP)
        self.queue = Queue()
        self._stopped = False
    ...
    def run(self):
        ...
        while not self.queue.full() :
            if not self.queue.empty() and not self._stopped:
...
class Shutter(QThread):
    ...
    def call_shutter(self, transition):
        self.queue.put([self.shutter, transition])

    def call_open(self):
        self.queue.put(self.open)
...
import time
time.sleep(3)
print "state after opening: %s" % s.current
